...
from algos import SASelector
from models import HistGBModel
...
# 準備 selector
selector = SASelector(days=DAYS_ROLL, iter_max=SA_ITER)

signals = []
# 逐日回測並顯示進度條
for predict_date in tqdm(dates, desc="SA backtest"):
    ...
    for feats in selected_sets:
        ...
        clf = HistGBModel(random_state=42)
        clf.fit(Xtr, ytr)
...
df_out.to_csv("signals/sa_signals.csv", index=False)
print(f"SA signals saved: {len(signals)} records → signals/sa_signals.csv")
